...
moving_average_init
	...
	LDR R6, =D							; Load the D value
	VMOV S4, R6							; Move to a floating point register
	VCVT.F32.U32 S4, S4					; Convert to a floating point value
	VLDR.F32 S5, =1						; Load value of 1 for division
	VDIV.F32 S4, S5, S4					; Store 1/D for multiplication later
	
	VLDR.F32 S5, =0						; Initialize average to 0
	
	MOV R6, R0							; Copy address of SP to R6
	MOV R7, R0							; Copy address of SP to R7
	ADD R7, R7, #(D*4)					; Increment to the end of the memory allocation
	
	VSTR.F32 S4, [R0, #(D*4)]			; Store 1/D
	VSTR.F32 S5, [R0, #(D+1)*4]			; Store AVG
	STR R0, [R0, #(D+2)*4]				; Store First memory address for last value in memory
	
setup_mem
	VLDR.F32 S8, =0
	VSTR.F32 S8, [R6]					; Setup the memory data to 0
	ADD R6, R6, #4						; Increment to the next memory location
	CMP R7, R6							; Compare if at the end of the memory allocation
	BNE setup_mem						; If not, loop
	
	BX LR								; Return to caller
	
moving_average
	; R0: Pointer to state
	; R6: Pointer to last value of MA filter
	; S0: New data value
	; S1: AVG
	; S4: 1/D
	; S6: Last value of MA to be removed
	
	VLDR.F32 S4, [R0, #D*4]				; Load 1/D
	VLDR.F32 S1, [R0, #(D+1)*4]			; Load AVG
	LDR R6, [R0, #(D+2)*4]				; Load address of last value of MA
	
	VLDR.F32 S6, [R6]					; Load last value of MA filter
	VSUB.F32 S1, S1, S6					; Remove the last value from MA filter
	VMUL.F32 S0, S0, S4					; Factor the new data by 1/D
	VADD.F32 S1, S1, S0					; Add the factored data to the AVG
	
	VSTR.F32 S0, [R6]					; Store the factored value in memory
	ADD R6, R6, #4						; Move the pointer to the next value in MA

	VSTR.F32 S1, [R0, #(D+1)*4]			; Store AVG
	
	ADD R7, R0, #(D*4)					; Get address of last memory allocation
	CMP R6, R7							; Compare if past last memory allocation
	SUBEQ R6, R6, #(D*4)				; If so, loop back around as a rotating array
	
	STR R6, [R0, #(D+2)*4]				; Store address of last value of MA
...
moving_average_test
	; S0: Next data value
	; R0: Address of SP
	; R4: Address of current position in data set
	; R5: Address of end of data set
	; R9: Branching address
	; R7, R8: Computation error condition values
	
	PUSH {LR}							; Push the Link Register onto the stack
	SUB SP, SP, #((D+3)*4)				; Allocate D*4-byte memory blocks for data storage
	MOV R0, SP							; R0 holds the address to reference the data
	
	LDR R9, =moving_average_init		; Load address of the init function
	BLX R9								; Branch and run init
	
	LDR R4, =test_vector				; Load 512 test data values
	MOV R5, R4							; Copy pointer to first data value in the data set
	ADD R5, #(512*4)					; Increment R5 to point to the end of the data set
	
	LDR R9, =moving_average				; Load address of the core function
	; VLDR S0, [R4]						; Load first data value from data set
	

ma_loop
	VLDR S0, [R4]						; Load next data value from data set
	ADD R4, R4, #4						; Increment pointer to next value in test vector
	
	PUSH {R4-R5}
	BLX R9								; Call moving_average function
	POP {R4-R5}
	
	
	
	LDR R7, =0x7F800000					; Load error comparison value for +/- INF and NaN
	VMOV R8, S0							; Store returned AVG for error comparison
	AND R8, R8, R7						; Get relevant part of AVG value for comparison
	CMP R7, R8							; Compare error value and AVG relevant part
	BEQ exit							; Branch to exit on error
	
	CMP R4, R5							; Compare if completed all elements in test_vector
	BNE ma_loop							; If not, loop moving_average

exit
	ADD SP, SP, #((D+3)*4)				; Deallocate D*4-byte memory blocks used for data storage
	POP {LR}							; Pop the Link Register from the stack
		
	BX LR								; Return program flow
	END									; End of function
